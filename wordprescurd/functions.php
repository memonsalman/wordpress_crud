...
{	
	...
	else
	{
	 	...
	 	$wpdb->update('wp_demo',$data,array('id' =>$id));
	}

	die();
}	

add_action("wp_ajax_test_ajax2", "test_ajax2");
...
{	
	if($_POST)
	{
		...
		$id=$_POST['id'];
		$data= array('id'=>$id);
		$wpdb->delete( 'wp_demo',$data);
	}
	die();
}	


add_action("wp_ajax_test_ajax3", "test_ajax3");
...
{	
	if($_POST)
	{
		global $wpdb;
		$id=$_POST['id'];
		$myrows = $wpdb->get_results( "SELECT * FROM wp_demo WHERE id=$id");
